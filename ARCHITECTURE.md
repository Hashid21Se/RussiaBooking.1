...
## 7. سجل القرار المعماري لتطبيقات الجوال (ADR: Mobile Framework - React Native vs Flutter)

### القرار المعتمد: React Native مع Expo SDK 51

#### مبررات الاختيار والمفاضلة الهندسية:
1. **إعادة استخدام منطق الأعمال بنسبة تزيد عن 75% (Code & Logic Sharing):**
   - مشاركة كافة أنواع TypeScript (`src/types.ts`)، عقود الـ API، دوال التحقق المالي والضرائب، وتحويل العملات مباشرة بين الويب وتطبيقات الجوال دون الحاجة لإعادة كتابتها بلغة Dart.
2. **سرعة التطوير والوصول للسوق (Time to Market):**
   - فريق العمل يمتلك كفاءة عالية في بيئة React/TypeScript، مما مكن من بناء وتجهيز تطبيق الجوال بكافة متطلبات المتاجر في وقت قياسي.
3. **تكامل المحافظ الرقمية الأصلية (Apple Pay & Google Pay):**
   - دعم مباشر ومستقر عبر حزم React Native الرسمية لبوابات الدفع الخليجية والمحلية (Mada, Tamara, Tap Payments).
4. **تحديثات التجاوز الفوري (OTA Updates عبر EAS Update):**
   - إمكانية إصلاح الأخطاء الطارئة وتحديث أسعار صرف العملات مباشرة على أجهزة المستخدمين دون الحاجة لانتظار دورة مراجعة المتاجر التي قد تستغرق أياماً.

#### متى كان يمكن اختيار Flutter؟
- لو كان التطبيق يتطلب رسوميات مخصصة ثلاثية الأبعاد مكثفة أو ألعاباً تفاعلية، لكن بالنسبة لتطبيق حجز فنادق فاخرة يعتمد على واجهات البيانات والنصوص والقسائم ودعم الـ RTL العربي، فإن React Native يقدم أداءً أصيلاً بنسبة 60-120 FPS ممتازاً وبحجم حزمة أصغر.

---

## 8. خارطة طريق التنفيذ (Execution Roadmap & Status)

- [x] **المرحلة 1:** التدقيق الهندسي الشامل، تقرير الفجوات، وتوثيق المعمارية في `ARCHITECTURE.md`.
- [x] **المرحلة 2:** مشاركة الأنواع DTOs ونماذج التحقق والمنطق المالي والأمني (Zero Overbooking & Lock Engine).
- [x] **المرحلة 3:** أمان البيانات والتشفير (AES-256-GCM، TOTP 2FA، تواقيع Webhook HMAC، وامتثال PDPL).
- [x] **المرحلة 4:** لوحة تحكم الفنادق وإدارة المنصة والتسويات المالية وسجلات التدقيق غير القابلة للتعديل.
- [x] **المرحلة 5:** بناء تطبيق الجوال من الصفر (React Native + Expo في `mobile/`) مع البصمة، Apple Pay، وقسائم بدون اتصال، وإعدادات المتاجر.
- [x] **المرحلة 6:** البنية التحتية والـ DevOps (Docker, docker-compose, CI/CD, Terraform, Prometheus/Grafana, ومراقبة Sentry).
- [x] **المرحلة 7:** التوثيق الشامل وخطط التعافي وما بعد الإطلاق (`DEPLOYMENT.md`, `README.md`, `SECURITY.md`, `QA_REPORT.md`, `POST_LAUNCH_PLAN.md`).
